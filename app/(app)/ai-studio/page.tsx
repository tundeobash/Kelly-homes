...
export default function AIStudioPage() {
  ...
  const [moreFurniture, setMoreFurniture] = useState(false) // "More furniture" toggle
  ...
  const loadProjects = async () => {
    try {
      ...
      // If projectIdParam exists, find that project and load its photo immediately
      ...
      // Set empty projects array to prevent UI crashes
      setProjects([])
    } finally {
  ...
  // When selectedProjectId changes, load photo from projects list first, then fetch full project data
  useEffect(() => {
    if (selectedProjectId) {
      // First, try to get imageUrl from already-loaded projects list
      ...
      if (projectFromList && projectFromList.imageUrl) {
        ...
        // Project exists but has no photo
        setCurrentImageUrl("")
      }

      // Then fetch full project data for AI designs
      const fetchProjectData = async () => {
        try {
          const res = await fetch(`/api/projects/${selectedProjectId}`)
          if (res.ok) {
            const project = await res.json()
            // Only update imageUrl if it's not already set from projects list
            // This prevents overwriting a valid URL with the same one
            ...
            // Load saved AI designs from project
  ...
  const handleGenerate = async () => {
    ...
    // Try to get imageUrl - use currentImageUrl if available, otherwise fetch from project
    let imageUrlToUse: string | null = null
    
    if (currentImageUrl && currentImageUrl.trim() !== "" && !currentImageUrl.startsWith("blob:")) {
      // Use currentImageUrl directly - no need to fetch project
      ...
      // currentImageUrl is missing - try to fetch from project
      if (process.env.NODE_ENV === "development") {
      ...
      try {
        ...
        if (projectRes.ok) {
          ...
          if (project.imageUrl && project.imageUrl.trim() !== "" && !project.imageUrl.startsWith("blob:")) {
            ...
            // Update currentImageUrl state for future use
    ...
    try {
      const res = await fetch("/api/ai/stage-room", {
        ...
        body: JSON.stringify({
          ...
          imageUrl: imageUrlToUse, // Pass imageUrl directly
          style: selectedStyle,
          prompt: refinePrompt || undefined,
          moreFurniture, // Pass "More furniture" toggle
        }),
      })
      ...
      if (data.success === true) {
        ...
        // CRITICAL: Use the returned imageUrl from API response (NOT the input currentImageUrl)
        ...
        if (returnedImageUrl) {
          ...
          // Always add fresh cache buster to force reload
          ...
          // Immediately prepend the newly generated design (this is the generated design, NOT the input)
          const newDesign = {
          ...
          // Prepend to existing designs (optimistic update)
          setGeneratedDesigns((prev) => {
            // Filter out any design with the same URL to avoid duplicates
            const filtered = prev.filter((d) => d.imageUrl.split("?")[0] !== returnedImageUrl.split("?")[0])
            return [newDesign, ...filtered]
          })
          
          // Then reload project to get all saved designs (including the new one with proper ID)
          if (selectedProjectId) {
            try {
              const projectRes = await fetch(`/api/projects/${selectedProjectId}`)
              if (projectRes.ok) {
                const projectData = await projectRes.json()
                // Use safe accessor: project.designs ?? project.aiDesigns ?? []
                ...
                // Update with saved designs (newest first)
                setGeneratedDesigns(savedDesigns)
              }
            } catch (error) {
              console.error("Failed to reload project designs:", error)
              // Keep showing the new image even if reload fails
            }
          }
        } else {
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  // Immediately load photo from projects list when project is selected
                  if (newProjectId) {
              ...
              <div className="space-y-4">
                <div className="relative w-full rounded-lg overflow-hidden border-2 border-gray-200">
                  {(() => {
                    ...
                    // Filter out stale generated_ URLs
                    ...
                    // Ensure we're NOT showing the input image (currentImageUrl)
                    // Only show /uploads/design-*.png files
                    ...
                    // Always add fresh cache buster
                    const displayUrl = `${cleanUrl}?v=${Date.now()}`
                    
                    return (
                      <img
                        key={displayUrl} // Force re-render with new URL
                        src={displayUrl}
                        ...
                        onError={(e) => {
                          ...
                          // Remove from state to prevent re-rendering
